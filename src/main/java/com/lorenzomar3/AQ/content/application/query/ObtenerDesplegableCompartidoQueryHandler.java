...
@Service
public class ObtenerDesplegableCompartidoQueryHandler {

    private final DesplegableCompartidoRepositoryPort desplegableCompartidoRepositoryPort;

    public ObtenerDesplegableCompartidoQueryHandler(DesplegableCompartidoRepositoryPort desplegableCompartidoRepositoryPort) {
        this.desplegableCompartidoRepositoryPort = desplegableCompartidoRepositoryPort;
    }

    @Transactional(readOnly = true)
    public DesplegableCompartidoFetchDTO handle(ObtenerDesplegableCompartidoQuery query) {
        ...
        return new DesplegableCompartidoFetchDTO(desplegableCompartido.getId(), desplegableCompartido.getTitulo(),
    ...
    private List<OpcionDeDesplegableCompartidoFetchDTO> mapearOpciones(DesplegableCompartido desplegableCompartido) {
        return desplegableCompartido.getListaDeOpciones().stream()
                .map(opcion -> new OpcionDeDesplegableCompartidoFetchDTO(opcion.getId(), opcion.getPregunta()))
                .toList();
    }
